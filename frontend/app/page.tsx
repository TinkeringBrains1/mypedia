...
function MapStatusIcon({ state }: { state: "mastered" | "struggling" | "current" | "upcoming" }) {
  const classes = {
    mastered: "border-[var(--accent-mastery)] bg-[var(--accent-mastery)] text-white",
    struggling: "border-[var(--accent-attention)] bg-[#ead8d2] text-[var(--accent-attention)]",
    current: "border-[var(--accent-growth)] bg-[#d7e6de] text-[#395c50]",
    upcoming: "border-[#aa9d8c] bg-[var(--surface)] text-[#8c8173]",
  }[state];
  return <span className={`relative z-10 mt-0.5 grid h-7 w-7 shrink-0 place-items-center rounded-full border-2 ${classes}`} aria-hidden="true">{state === "mastered" && <svg viewBox="0 0 16 16" className="h-3.5 w-3.5 fill-none stroke-current" strokeWidth="2.3"><path d="m3 8 3 3 7-7" /></svg>}{state === "struggling" && <svg viewBox="0 0 16 16" className="h-3.5 w-3.5 fill-none stroke-current" strokeWidth="2"><path d="M8 3v5M8 11.5v.1" strokeLinecap="round" /><path d="M8 1.7 14.4 14H1.6L8 1.7Z" strokeLinejoin="round" /></svg>}{state === "current" && <span className="h-2.5 w-2.5 rounded-full bg-current shadow-[0_0_0_4px_rgba(91,140,123,.16)]" />}{state === "upcoming" && <svg viewBox="0 0 16 16" className="h-3.5 w-3.5 fill-none stroke-current" strokeWidth="1.8"><path d="M5 3.5 10 8l-5 4.5" strokeLinecap="round" strokeLinejoin="round" /></svg>}</span>;
}

function ProfilePanel({ memory }: { memory: LearningMemoryApi | null }) {
  const graph = memory?.knowledge_profile.concept_graph_position;
  return <aside className="fixed inset-y-0 right-0 z-20 hidden w-[285px] border-l border-[var(--border)] bg-[#eee7da] px-7 py-7 xl:block"><p className="text-[11px] font-semibold uppercase tracking-[.14em] text-[var(--muted)]">Your learning map</p><h2 className="tutor-display mt-2 text-3xl font-semibold">Maths</h2><p className="mt-2 text-sm leading-6 text-[var(--muted)]">A simple view of the ideas you are working through.</p><ol className="mt-10 space-y-0">{conceptNodes.map((node, index) => { const mastered = graph?.mastered_nodes.includes(node.id); const struggling = graph?.struggling_nodes.includes(node.id); const current = graph?.current_node === node.id; const state = mastered ? "Understood" : struggling ? "Needs practice" : current ? "Current" : "Up next"; const iconState: "mastered" | "struggling" | "current" | "upcoming" = mastered ? "mastered" : struggling ? "struggling" : current ? "current" : "upcoming"; return <li key={node.id} className="relative flex gap-4 pb-7 last:pb-0">{index < conceptNodes.length - 1 && <span className="absolute left-[13px] top-7 h-[calc(100%-4px)] border-l border-[#c8baa7]" />}<MapStatusIcon state={iconState} /><span><strong className="block text-sm font-medium">{node.label}</strong><span className="mt-0.5 block text-xs text-[var(--muted)]">{state}</span></span></li>; })}</ol><p className="mt-10 border-t border-[var(--border)] pt-5 text-xs leading-5 text-[var(--muted)]">Each step becomes clearer through practice, not speed.</p></aside>;
}
